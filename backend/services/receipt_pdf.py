...
- Branded FUOYE header with session info
...
# ── Colour palette (matching FUOYE forest / lime theme) ────────────────────────────────────────
...
def _header_block(receipt: dict) -> list:
    ...
    fuoye_style = ParagraphStyle(
        "FUOYETitle", fontName="Helvetica-Bold", fontSize=26,
        textColor=FOREST, alignment=TA_CENTER, spaceAfter=2 * mm,
        leading=30
    )
    elements.append(Paragraph("<i>FUOYE</i>", fuoye_style))
    ...
    elements.append(Paragraph("HOSTEL MANAGEMENT PORTAL", sub_style))
...
def _reference_badge(receipt: dict) -> list:
    """Prominent payment reference block."""
    elements = []

    ref = receipt.get("hms_reference", "—")
    badge_data = [
        [Paragraph("PAYMENT REFERENCE", ParagraphStyle(
...
def _footer() -> list:
    ...
    elements.append(Paragraph(
        f"Generated by FUOYE — Hostel Management Portal  •  {now}", footer_style
    ))
    ...
    elements.append(Paragraph("FUOYE  |  HOSTEL MANAGEMENT PORTAL  |  PAYMENT VERIFIED", brand_style))
...
def generate_receipt_pdf(receipt: dict) -> bytes:
    ...
    doc = SimpleDocTemplate(
        ...
        title="FUOYE Payment Receipt",
        author="FUOYE — Hostel Management Portal",
    )
